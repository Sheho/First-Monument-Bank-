        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            ...
            <div className="flex items-center gap-4">
              <Link to="/login" className="text-xs font-medium text-slate-900 hover:text-blue-600 transition-colors hidden sm:block">Sign In</Link>
              <Link to="/open-account" className="hidden md:inline-flex bg-gradient-to-r from-blue-600 to-blue-800 hover:from-blue-700 hover:to-blue-900 text-white px-4 py-1.5 rounded-full text-xs font-semibold transition-all shadow-[0_0_15px_rgba(37,99,235,0.3)] hover:shadow-[0_0_25px_rgba(37,99,235,0.5)]">
                Open an Account
              </Link>
              <button 
      ...
      <div className={`md:hidden fixed top-0 left-0 h-full w-[280px] bg-white shadow-[4px_0_30px_rgba(0,0,0,0.8)] z-[70] transform transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex flex-col h-full px-6 py-8">
          ...
          <div className="pt-6 border-t border-slate-200 flex flex-col gap-4 mt-auto">
            <Link to="/login" onClick={() => setIsMobileMenuOpen(false)} className="text-sm font-medium text-slate-900 text-center border border-slate-300 rounded-full py-3 hover:bg-slate-50 transition-all">Sign In</Link>
            <Link to="/open-account" onClick={() => setIsMobileMenuOpen(false)} className="text-sm font-medium text-white text-center bg-gradient-to-r from-blue-600 to-blue-800 rounded-full py-3 hover:opacity-90 transition-all">Open an Account</Link>
          </div>
        </div>
      </div>
